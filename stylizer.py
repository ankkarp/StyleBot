...
def load_image(img_path, max_size=400, shape=None):
    ...
    size = shape if shape else min(max_size, max(image.shape))
...
def stylize(chat_id, steps=100):
    ...
    for _ in tqdm(range(steps)):
        ...
        total_loss.backward()
        optimizer.step()
    cv2.imwrite(f'res_{chat_id}.png', (im_convert(target) * 255).astype(np.uint8))